...
import {
    ...
    Image, Modal, Dimensions, SafeAreaView,
} from "react-native";
    ...
    return (
        <View style={tc.card}>
            ...
            <View style={tc.restHeader}>
                <Image
                    source={{ uri: card.restaurantImage || "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=200" }}
                    style={tc.restAvatar}
                />
                <View style={{ flex: 1 }}>
                    <Text style={tc.restName} numberOfLines={1}>{card.restaurantName}</Text>
                    <View style={tc.restMeta}>
                        <Ionicons name="star" size={11} color="#F59E0B" />
                        <Text style={tc.restRating}>{card.restaurantRating?.toFixed(1)}</Text>
                        <Text style={tc.restSep}>•</Text>
                        <Text style={tc.restCity} numberOfLines={1}>{card.restaurantCity}</Text>
                    </View>
                </View>
            </View>

            {/* Table image — tap to open gallery */}
            <TouchableOpacity onPress={() => { setGalleryIdx(0); setShowGallery(true); }} activeOpacity={0.9}>
                <Image
                    source={{ uri: card.tableImage || card.restaurantImage || "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=600" }}
                    style={tc.tableImg}
                    resizeMode="cover"
                />
                {/* Type badge */}
                <View style={[tc.typeBadge, { backgroundColor: cfg.bg }]}>
                    <Text style={[tc.typeBadgeText, { color: cfg.color }]}>{cfg.label}</Text>
                </View>
                {/* Gallery indicator */}
                {allImages.length > 1 && (
            ...
            <View style={tc.body}>
                ...
                {card.description ? (
                    <Text style={tc.desc} numberOfLines={2}>{card.description}</Text>
                ) : null}
                ...
                <View style={gal.overlay}>
                    <SafeAreaView style={{ flex: 1 }}>
                        ...
                        <View style={gal.header}>
                            <Text style={gal.headerTitle}>{card.tableName} — {card.restaurantName}</Text>
                            <TouchableOpacity onPress={() => setShowGallery(false)} style={gal.closeBtn}>
                        ...
                        <View style={gal.dots}>
                            {allImages.map((_, i) => (
                                <View key={i} style={[gal.dot, i === galleryIdx && gal.dotActive]} />
                            ))}
                        </View>
                        ...
                        <TouchableOpacity
                            style={gal.bookBtn}
                            onPress={() => { setShowGallery(false); onBook(card); }}
                            activeOpacity={0.85}
                        >
...
export default function ChatScreen() {
    ...
    const sendMessage = async (customText?: string) => {
        ...
        try {
            const { response, session: newSession } = await ambleAI.chat(text, session);
        ...
        return (
            <View style={{ marginBottom: 12 }}>
                <View style={[s.msgRow, isUser ? s.msgRowUser : s.msgRowAI]}>
                    {/* AI avatar */}
                    {!isUser && (
                        <LinearGradient colors={["#FF6B35", "#FFD700"]} style={s.aiAvatar} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
                            <Ionicons name="sparkles" size={14} color="#fff" />
                        </LinearGradient>
                    )}

                    {/* Bubble */}
                    <View style={[s.bubble, isUser ? s.bubbleUser : s.bubbleAI, { maxWidth: SW * 0.72 }]}>
                ...
                {!isUser && item.restaurants && item.restaurants.length > 0 && !item.tableCards && (
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}
                        ...
                        {item.restaurants.map((r: any) => (
                            <TouchableOpacity
                                key={r._id}
                                style={sr.card}
                                onPress={() => router.push({ pathname: `/restaurant/${r._id}` as any })}
                                activeOpacity={0.85}
                            >
                                <Image source={{ uri: r.images?.[0] || r.image }} style={sr.img} resizeMode="cover" />
                                <View style={sr.info}>
    ...
    return (
        ...
        >
            {/* Header */}
            <LinearGradient colors={["#FF6B35", "#FF8C55"]} style={s.header} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
            ...
            {loading && (
                <View style={s.typing}>
                    <LinearGradient colors={["#FF6B35", "#FFD700"]} style={s.typingAvatar} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.suggestions}>
        {suggestions.map((s_, i) => (
            <TouchableOpacity key={i} style={s.suggestionChip} onPress={() => sendMessage(s_)} activeOpacity={0.7}>
            ...
            <View style={s.inputBar}>
                <View style={s.inputWrap}>
                    ...
                    <TouchableOpacity
                        ...
                        activeOpacity={0.8}
                    >
                        <Ionicons name="send" size={18} color={inputText.trim() && !loading ? "#fff" : "#C4C4C4"} />
                    </TouchableOpacity>
                </View>
            </View>
        </KeyboardAvoidingView>
...
const s = StyleSheet.create({
    ...
    headerAvatar: { width: 38, height: 38, borderRadius: 19, backgroundColor: "#fff", alignItems: "center", justifyContent: "center" },
    headerTitle:  { fontSize: 16, fontWeight: "800", color: "#fff" },
    headerOnline: { flexDirection: "row", alignItems: "center", gap: 4, marginTop: 1 },
    onlineDot:    { width: 6, height: 6, borderRadius: 3, backgroundColor: "#4ADE80" },
    headerSub:    { fontSize: 11, color: "rgba(255,255,255,0.85)" },
    resetBtn:     { width: 36, height: 36, borderRadius: 18, backgroundColor: "rgba(255,255,255,0.2)", alignItems: "center", justifyContent: "center" },

    msgRow:       { flexDirection: "row", alignItems: "flex-end", gap: 6 },
    msgRowUser:   { justifyContent: "flex-end" },
    msgRowAI:     { justifyContent: "flex-start" },
    aiAvatar:     { width: 28, height: 28, borderRadius: 14, alignItems: "center", justifyContent: "center" },
    userAvatar:   { width: 28, height: 28, borderRadius: 14, backgroundColor: "#374151", alignItems: "center", justifyContent: "center" },
    bubble:       { padding: 12, borderRadius: 18 },
    bubbleUser:   { backgroundColor: PRIMARY, borderBottomRightRadius: 4 },
    bubbleAI:     { backgroundColor: "#fff", borderBottomLeftRadius: 4, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
    ...
    qrRow:        { flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 6, marginLeft: 34 },
    qrChip:       { backgroundColor: "#fff", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, borderWidth: 1.5, borderColor: PRIMARY },
    ...
    typingBubble: { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "#fff", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 16 },
...
suggestions: {
    ...
    suggestionChip: { backgroundColor: "#fff", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1, borderColor: "#E5E7EB" },
    suggestionText: { fontSize: 13, color: "#374151", fontWeight: "500" },

    inputBar:     { backgroundColor: "#fff", borderTopWidth: 1, borderTopColor: "#F3F4F6", padding: 12 },
    inputWrap:    { flexDirection: "row", alignItems: "flex-end", gap: 8, backgroundColor: "#F3F4F6", borderRadius: 24, paddingHorizontal: 14, paddingVertical: 8 },
    input:        { flex: 1, fontSize: 15, color: "#1A1A1A", maxHeight: 100, paddingTop: 2 },
    sendBtn:      { width: 34, height: 34, borderRadius: 17, backgroundColor: PRIMARY, alignItems: "center", justifyContent: "center" },
    sendBtnOff:   { backgroundColor: "#E5E7EB" },
});

// Table card styles
const tc = StyleSheet.create({
    card:         { backgroundColor: "#fff", borderRadius: 16, overflow: "hidden", marginBottom: 12, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 8, elevation: 3, width: SW - 68 },
    restHeader:   { flexDirection: "row", alignItems: "center", gap: 8, padding: 10, borderBottomWidth: 1, borderBottomColor: "#F3F4F6" },
    restAvatar:   { width: 32, height: 32, borderRadius: 8 },
    restName:     { fontSize: 13, fontWeight: "700", color: "#1A1A1A" },
    restMeta:     { flexDirection: "row", alignItems: "center", gap: 3, marginTop: 1 },
    ...
    typeBadge:    { position: "absolute", top: 8, left: 8, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 20 },
    typeBadgeText:{ fontSize: 11, fontWeight: "700" },
    galleryBadge: { position: "absolute", top: 8, right: 8, flexDirection: "row", alignItems: "center", gap: 3, backgroundColor: "rgba(0,0,0,0.5)", paddingHorizontal: 7, paddingVertical: 3, borderRadius: 12 },
    galleryCount: { fontSize: 11, color: "#fff", fontWeight: "600" },
    body:         { padding: 12 },
    titleRow:     { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 6 },
    tableName:    { fontSize: 15, fontWeight: "800", color: "#1A1A1A" },
    availBadge:   { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#ECFDF5", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 12 },
    availDot:     { width: 6, height: 6, borderRadius: 3, backgroundColor: "#22C55E" },
    availText:    { fontSize: 11, fontWeight: "600", color: "#065F46" },
    featureChip:  { backgroundColor: "#F3F4F6", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, marginRight: 6 },
    ...
    bookBtnInner: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 12, paddingVertical: 8 },
    bookBtnText:  { fontSize: 13, fontWeight: "700", color: "#fff" },
});

// Simple restaurant card styles
const sr = StyleSheet.create({
    card:   { width: 160, backgroundColor: "#fff", borderRadius: 12, marginRight: 10, overflow: "hidden", shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
...
const gal = StyleSheet.create({
    ...
    headerTitle:  { fontSize: 14, fontWeight: "700", color: "#fff", flex: 1, marginRight: 8 },
    ...
    bookBtn:      { marginHorizontal: 20, marginBottom: 20, borderRadius: 14, overflow: "hidden" },
